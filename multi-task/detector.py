...
def num_line(lane_seg):
    global depth
    image = (lane_seg.numpy() * 255).astype(np.uint8)
...
def torch_thread(weights, img_size, result_queue, conf_thres=0.45, iou_thres=0.45):
    ...
    while not exit_signal:
        if run_signal:
            ...
            results = model.predict(img, save=False, imgsz=img_size, conf=conf_thres, iou=iou_thres)  # 不需要show
            # det, road_seg, lane_seg = results[2], results[0][0].cpu(), results[1][0].cpu()
            det, lane_seg = results[0], results[1][0].cpu()
            im, lane_lines_count, left_and_right_Point = num_line(lane_seg)

            # cv2.imwrite('lane.png', lane_seg.numpy() * 255)
            # 组合mask
            # seg_mask = np.zeros((det[0].orig_shape[0], det[0].orig_shape[1], 3), dtype=np.uint8)
            ...
            # alpha = 0.5
            # image_net1[:, :, :3][np.any(color_road != [0, 0, 0], axis=-1)] = (1 - alpha) * image_net1[:, :, :3][
            #     np.any(color_road != [0, 0, 0], axis=-1)] + alpha * color_road[
            #                                                               np.any(color_road != [0, 0, 0], axis=-1)]
            # image_net1[:, :, :3][np.any(color_lane != [0, 0, 0], axis=-1)] = (1 - alpha) * image_net1[:, :, :3][
            #     np.any(color_lane != [0, 0, 0], axis=-1)] + alpha * color_lane[
            #                                                               np.any(color_lane != [0, 0, 0], axis=-1)]
            cv2.namedWindow('lane_seg', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('lane_seg', 1280, 720)
            cv2.imshow("lane_seg", im)
            # cv2.imshow("road_seg", (road_seg.numpy() * 255).astype(np.uint8))
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break

            # ZED CustomBox format (with inverse letterboxing tf applied)
            detections = detections_to_custom_box(det[0].boxes, image_net)
            lock.release()
            run_signal = False
            result_queue.put([lane_lines_count, left_and_right_Point, det[0],im])
        sleep(0.01)
...
def main():
    ...
    while not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            #         f"{right_point[0]:.3f} {right_point[1]:.3f} {right_point[2]:.3f},{round(lane_width, 3)}\n")
            # # 写入检测到的目标信息
            # for cls, objs, zed_obj in zip(result[2].boxes.cls, result[2].boxes.xywh, objects.object_list):
            #     point = point_cloud.get_data()[
            #                 int(round(objs.cpu().numpy()[1], 0)), int(round(objs.cpu().numpy()[0], 0))][:3]
            #     file.write(
            #         f"label:{label_list[int(cls)]},{point[0]:.3f} {point[1]:.3f} {point[2]:.3f},{zed_obj.id}\n")
            # # endregion
            ...
            cv2.imwrite(f"save_imgs/{count}.jpg",image_left_ocv)
            cv2.imwrite(f"save_imgs/{count}--{result[0]}.png",result[3])
            count+=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--info_txt_path', type=str, default='zed_information.txt')
